...
| Tech Domain   | Python + FastAPI + Neo4j + MongoDB + Redis + LangGraph + Groq                            |
...
- Three-stage entity resolution — Stage 1 Jaro-Winkler lexical, Stage 2
  sentence-transformers embedding similarity, Stage 3 Neo4j graph
  neighborhood
...
- Hybrid retrieval — BM25 full-text search parallel with
  sentence-transformers vector search, fused via Reciprocal Rank Fusion
...
- LangGraph coordinator with three specialist agents — Graph Traversal,
...
| Complex query full answer                    | \< 45 seconds end-to-end                                                  | LangGraph coordinator measured on benchmark queries                         | Takes \> 90 seconds consistently                             |
...
> Python + FastAPI throughout — not Node.js/TypeScript. If an AI
> suggests Node.js, Express, or TypeScript for any module here, that is
> wrong. Database is MongoDB (not Postgres) — this was reconsidered and
> MongoDB was kept.*
>
> **2.1 — Architecture Design**
...
> **WHY:** Single Python + FastAPI process organised into feature-based
...
→ FastAPI route handler validates and acknowledges immediately under
...
sentence-transformers stored in MongoDB Atlas Vector Search → Query
arrives at /api/v1/query → Query complexity classified — simple queries
bypass agent pipeline → Hybrid retrieval: BM25 + vector search + Neo4j
graph expansion, fused via RRF → Agent pipeline: LangGraph coordinator
...
LangGraph agent pipeline → Graph Traversal Specialist searches Neo4j for
PostgreSQL-related decision nodes → Causal Inference Specialist traces
what decisions came before and after the PostgreSQL nodes → Synthesis
Specialist assembles findings → Five-step chain: extract relevant nodes
→ reason about causal chain → synthesise answer → self-reflect for gaps
→ format with citations → Hallucination guard cross-references each
claim → Returns answer: "In Q3 2022, the team decided to move from
PostgreSQL to MongoDB \[Slack thread link, commit SHA\]. The primary
reason cited was schema flexibility for varying event types \[GitHub PR
\#147\]. This was reversed in Q1 2023 for the analytics module which
moved back to PostgreSQL for JOIN performance \[Decision node, document
upload\]." with confidence 0.87.
...
|---------------------|---------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Runtime             | Python 3.12                                                   | Standard language for the AI/RAG ecosystem — LangGraph, sentence-transformers, and most retrieval libraries are Python-native, reducing glue code versus a JS agent stack. Consolidates the candidate's portfolio around one primary backend language ahead of placements. Type safety via mypy strict mode + Pydantic models across graph schema, entity schemas, and API contracts. Tradeoff: GitHub/Slack SDKs (PyGithub, slack-bolt) are slightly less mature here than their JS counterparts — accepted. | Node.js + TypeScript — the original choice (Octokit/Bolt SDKs are more mature there), reconsidered mid-build; Python's AI-native tooling and portfolio consolidation outweighed the thinner GitHub/Slack SDK support. Go — no LangChain or LangGraph equivalent. Plain Python without type hints — no type safety on graph queries is dangerous. |
| API Framework       | FastAPI                                                       | Most widely used async Python web framework for APIs — every interviewer knows it. Dependency-injection pattern is exactly right for the auth → rate limit → validation → route handler chain. Auto-generated OpenAPI docs. No magic.                                                                                                                                                                                                                                                                         | Flask — simpler but lacks native async and built-in validation, more boilerplate at this scope. Django — too heavy/opinionated for an API-only service. Express (Node.js) — the original choice, dropped along with the runtime switch.                                                                                                          |
...
| Embeddings          | sentence-transformers (all-MiniLM-L6-v2)                      | Runs entirely in-process — zero API cost, zero network latency, works offline. 384-dimensional embeddings sufficient for entity resolution and hybrid retrieval at this scale. Model loads once on startup, subsequent calls are fast.                                                                                                                                                                                                                                                                        | OpenAI text-embedding-3-small — API cost at ingestion volume, network latency on every embedding. Cohere Embed — costs money.                                                                                                                                                                                                                    |
...
| Agent Orchestration | LangGraph                                                     | Stateful graph-based coordination — coordinator decides dynamically which specialist to call next based on query type and intermediate results. Handles agent loops (retry when confidence low), conditional branching, and shared state across the five-step prompt chain. LangChain is correct for simple linear chains. LangGraph is correct when agents make decisions about what to do next.                                                                                                             | LangChain sequential chain — fixed order, cannot branch based on what specialists find. AutoGen — less control over tool calling and safety layers. Building from scratch — LangGraph handles the state machine correctly, no need to reinvent.                                                                                                  |
| File Parsing        | pypdf + python-docx                                           | Both are pure Python packages — no Java sidecar, no Docker container, no extra service. pypdf handles PDF text extraction. python-docx extracts DOCX text directly. Both work in the same Python process.                                                                                                                                                                                                                                                                                                     | Apache Tika — Java sidecar in a Python project, adds a service to Docker Compose, complex setup. PDFKit — for creating PDFs not parsing them. Textract AWS — costs money, AWS lock-in.                                                                                                                                                           |
| File Watching       | watchdog                                                      | Cross-platform file system watcher for local file connector. Handles symlinks, macOS FSEvents, Linux inotify, Windows FSEvents. Battle-tested — the standard Python file-watching library.                                                                                                                                                                                                                                                                                                                    | Manual inotify polling — reinventing what watchdog already solves. Watchman — requires separate installation, overkill.                                                                                                                                                                                                                          |
| Auth                | Google OAuth 2.0 + Authlib + JWT                              | Google OAuth: one button, no password management, no email verification. Authlib handles the OAuth flow with the Google strategy in ~50 lines. JWT with workspace-scoped claims for contribution tracking and expert routing.                                                                                                                                                                                                                                                                                 | Email/password auth — weeks of work for password reset, email verification, rate limiting brute force. Auth0 — costs money. NextAuth — too Next.js specific.                                                                                                                                                                                     |
...
| Containers          | Docker + Docker Compose                                       | One command starts all 5 services: Python API + worker, MongoDB, Neo4j, Redis, Ollama.                                                                                                                                                                                                                                                                                                                                                                                                                        | Podman — less universal. No containers — not reproducible across machines.                                                                                                                                                                                                                                                                       |
...
sentence-transformers embeddings for all ingested content. Separate from
...
| embedding       | number\[384\] | sentence-transformers all-MiniLM-L6-v2 vector — stored as Atlas Vector Search field |
...
| agent_status        | { agent: string, status: "running"\|"complete", summary?: string }                                                    | When LangGraph coordinator assigns to a specialist or specialist completes |
...
Google OAuth 2.0 via Authlib's Google OAuth client. Flow: user clicks
Sign In with Google → Google returns authorisation code → POST
/auth/google → exchange for Google user profile → create or update user
in MongoDB → issue JWT access token (15 minute expiry) + refresh token
(7 day expiry, stored in Redis). Rule R-53: JWT tokens have expiry and
refresh rotation.
...
| A05 Security Misconfiguration | CORS explicitly configured — frontend origin only. Security headers via secure-headers middleware: X-Content-Type-Options, X-Frame-Options, CSP. No default credentials.                          |
| A06 Vulnerable Components     | pip-audit runs in CI. Dependabot alerts enabled. Dependencies pinned in requirements.txt.                                                                                                         |
...
| A08 Software Integrity        | requirements.txt committed with pinned versions. Docker image built from pinned base image. No external scripts executed at runtime.                                                              |
| A09 Logging Failures          | structlog structured logging on every request. Errors logged with request_id. No tokens, keys, or raw user content in logs.                                                                       |
...
load: add more Python worker processes consuming from the same Redis
...
|----------------------------------------|-----------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------|
| Neo4j connection lost                  | GET /health returns neo4j: "unhealthy"                                | Driver-level retry with exponential backoff (3 retries). Ingestion worker pauses and retries — Redis Streams messages remain unconsumed until reconnected. Queries return 503 SERVICE_UNAVAILABLE with helpful message. | \< 5 min with auto-retry |
...
| sentence-transformers model not loaded | Entity resolution Stage 2 fails. Hybrid retrieval vector stage fails. | Model preloaded on server startup. If startup load fails, server logs clear error and exits — forces redeploy with correct model file. Never partial startup.                                                           | Redeploy required        |
...
| MongoDB      | Document storage — raw events, embeddings, generated content                      | Raw ingested payloads, sentence-transformers embeddings (via Atlas Vector Search), AI-generated document text, ingestion processing status       | Graph traversal, relationship queries, anything requiring JOIN-like operations                                               |
...
|----------|-----------------------|------------------------------------------|--------------------------------------------------------------------------------------------|-------------------------------------|
| 1        | GitHub Actions        | FastAPI POST /ingest/github/webhook      | GitHub webhook payload with X-Hub-Signature-256 header                                     | —                                   |
| 2        | FastAPI middleware    | HMAC validator                           | Validates signature against GITHUB_WEBHOOK_SECRET. Rejects immediately if invalid.         | \< 5ms                              |
| 3        | FastAPI route handler | Normaliser (app/ingestion/normaliser.py) | Raw GitHub payload converted to UnifiedEvent schema                                        | \< 10ms                             |
| 4        | FastAPI route handler | MongoDB raw_events (INSERT)              | UnifiedEvent stored with status "pending". external_id checked for duplicates.             | \< 20ms                             |
| 5        | FastAPI route handler | Redis Streams XADD (omnirag:events)      | raw_event MongoDB \_id published to stream. 202 Accepted returned to GitHub.               | \< 5ms. Total API response: \< 50ms |
...
| 8        | Consumer Worker       | sentence-transformers (in-process)       | Embed the content text. No API call — local inference.                                     | \< 100ms warm, \< 200ms cold        |
...
| **Step**       | **From**                       | **To**                                                     | **Data**                                                                                       | **Latency**                                   |
|----------------|--------------------------------|------------------------------------------------------------|------------------------------------------------------------------------------------------------|-----------------------------------------------|
| 1              | React frontend                 | FastAPI GET /query/stream?question=...                     | Question string + optional filters. JWT validated.                                             | —                                             |
| 2              | FastAPI route                  | Redis cache (GET)                                          | Hash of question + filters checked. Cache hit → return immediately.                            | \< 5ms. If hit: \< 50ms total response.       |
| 3 (cache miss) | FastAPI route                  | QueryTypeClassifier (Groq LLaMA 3.1 8B)                    | Classify query: FACTUAL / CAUSAL / TEMPORAL / EXPERTISE / COMPLEX                              | \< 1 second — fast model                      |
...
| 5              | Query service                  | RRF fusion (app/retrieval/rrf.py)                          | BM25 and vector rankings combined. Pure in-memory computation.                                 | \< 5ms                                        |
...
| 7 (complex)    | Query service                  | LangGraph Coordinator                                      | Full agent pipeline: coordinator assigns specialists. agent_status SSE events emitted.         | Streaming starts \< 3s. Full answer \< 45s.   |
| 8              | Agents                         | Groq LLaMA 3.1 70B                                         | Specialist reasoning. Tokens stream back in real time.                                         | Groq: 500 tok/sec                             |
| 9              | Answer returned                | HallucinationGuard (app/retrieval/hallucination_guard.py)  | Every factual claim checked against retrieved source node IDs. Uncorroborated claims stripped. | \< 50ms — deterministic Python                |
| 10             | Answer validated               | ConfidenceScorer (app/retrieval/confidence_scorer.py)      | Confidence derived from source_count, recency, contradiction_count, verification_status.       | \< 30ms — deterministic formula + Neo4j reads |
...
> **PART 03 — Engineering Blueprint**
>
> **ℹ** *mypy strict mode throughout. Feature-based folder structure —
> Rule R-15. All database access through repository classes — Rule R-48.
> No file longer than 300 lines — Rule R-16. Dependencies flow downward
> only — routes call services call repositories, never the reverse.*
>
> **3.1 — Folder Structure**
>
...
| **Path**                                                | **Contents**                                                                                                                                                         | **Rule**    |
|---------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| omnirag/                                                | Project root — docker-compose.yml, Makefile, README.md, pyproject.toml, requirements.txt                                                                             |             |
| omnirag/src/                                            | All Python source code                                                                                                                                               | R-15        |
| omnirag/app/main.py                                     | FastAPI app factory — registers routers, middleware, error handler. Under 80 lines.                                                                                  | R-39        |
| omnirag/app/server.py                                   | ASGI entry point — uvicorn target, binds port, starts stream consumers on FastAPI startup event.                                                                     |             |
| omnirag/app/config.py                                   | Pydantic Settings singleton — reads all env vars, validates on import via BaseSettings, raises with clear message if missing. All other modules import from here.    | R-19, R-36  |
| omnirag/app/shared/                                     | Cross-cutting concerns used by multiple features                                                                                                                     |             |
| omnirag/app/shared/schemas.py                           | All shared Pydantic models and types — UnifiedEvent, GraphNode, QueryResult, Citation, etc. Single source of truth.                                                  | R-33        |
| omnirag/app/shared/constants.py                         | All named constants — ENTITY_MERGE_THRESHOLD, GRAPH_EXPANSION_HOPS, GROQ_MODEL_FAST, etc. No magic numbers anywhere else.                                            | R-34        |
| omnirag/app/shared/errors.py                            | Custom error classes — OmniRAGError base, ValidationError, NotFoundError, GraphEmptyError, etc. Standard error codes.                                                | R-41        |
| omnirag/app/shared/logger.py                            | structlog logger configuration. All modules import logger from here — never configure logging twice.                                                                 | R-42        |
| omnirag/app/shared/middleware.py                        | request-id injection, error handler, CORS, secure-headers middleware, request logging.                                                                               |             |
| omnirag/app/database/                                   | Database connection setup and shared utilities                                                                                                                       |             |
| omnirag/app/database/neo4j.py                           | Neo4j driver singleton (official neo4j Python driver) — connection pool, session factory. Loaded once on startup.                                                    |             |
| omnirag/app/database/mongodb.py                         | Motor (async PyMongo) connection + all collection models.                                                                                                            |             |
| omnirag/app/database/redis.py                           | redis-py client singleton — used by cache, streams, pub/sub.                                                                                                         |             |
| omnirag/app/ingestion/                                  | Everything about receiving events from external sources                                                                                                              |             |
| omnirag/app/ingestion/routes.py                         | FastAPI router — POST /ingest/github/webhook, POST /ingest/slack/webhook, POST /ingest/files. Validates signatures, publishes to Redis Streams. Under 80 lines.      | R-31        |
| omnirag/app/ingestion/normaliser.py                     | Converts raw source payloads to unified UnifiedEvent schema. Pure functions, no side effects. One function per source type.                                          | R-39        |
| omnirag/app/ingestion/privacy_classifier.py             | Calls Ollama llama3 locally via the Python Ollama client to classify content privacy level. Four levels. Sensitive content excluded before graph write.              |             |
| omnirag/app/ingestion/stream_consumer.py                | redis-py Streams XREADGROUP consumer — processes events from ingestion queue, calls normaliser then entity resolution. Handles acknowledgment and dead letter.       |             |
| omnirag/app/ingestion/deduplication.py                  | Checks MongoDB raw_events collection for existing external_id before processing. Prevents same GitHub commit processed twice.                                        |             |
| omnirag/app/ingestion/test_ingestion.py                 | Pytest tests co-located with source — Rule R-17.                                                                                                                     | R-17        |
| omnirag/app/entity_resolution/                          | Three-stage entity resolution pipeline                                                                                                                               |             |
| omnirag/app/entity_resolution/stage1_lexical.py         | Jaro-Winkler distance matching. Returns candidates above 0.70 threshold. Pure function — no database calls.                                                          |             |
| omnirag/app/entity_resolution/stage2_semantic.py        | Cosine similarity of sentence-transformers embeddings. Returns candidates above 0.75 threshold.                                                                      |             |
| omnirag/app/entity_resolution/stage3_graph.py           | Neo4j graph neighborhood analysis — shared neighbor count as confidence signal. Handles cross-system identity resolution.                                            |             |
| omnirag/app/entity_resolution/resolver.py               | Orchestrates all three stages. Decision logic: auto-merge above 0.85, human review queue 0.60-0.85, new entity below 0.60.                                           |             |
| omnirag/app/entity_resolution/merge_undo.py             | Records every merge as a reversible event. Undo function restores both nodes with original relationships.                                                            |             |
| omnirag/app/entity_resolution/repository.py             | Neo4j queries for entity lookup, merge, undo. No business logic.                                                                                                     | R-48        |
| omnirag/app/entity_resolution/test_entity_resolution.py | Unit tests including adversarial test set — similar-but-different names that must NOT merge.                                                                         | R-17, R-45  |
| omnirag/app/graph/                                      | Temporal knowledge graph write operations                                                                                                                            |             |
| omnirag/app/graph/writer.py                             | Writes resolved events to Neo4j — creates nodes with temporal properties, creates relationships, detects contradictions.                                             |             |
| omnirag/app/graph/contradiction_detector.py             | Detects three contradiction types: direct_factual, temporal, cross_source_ownership. Creates Contradiction nodes when found.                                         |             |
| omnirag/app/graph/classifier.py                         | Five-type message classifier using Groq LLaMA 3.1 8B — DECISION, QUESTION, SOLUTION, CONTEXT, NOISE.                                                                 |             |
| omnirag/app/graph/repository.py                         | Neo4j queries for graph reading — node lookup, relationship traversal, temporal queries. No business logic.                                                          | R-48        |
| omnirag/app/graph/test_graph.py                         | Unit tests for temporal query logic and contradiction detection.                                                                                                     | R-17        |
| omnirag/app/retrieval/                                  | Hybrid retrieval pipeline                                                                                                                                            |             |
| omnirag/app/retrieval/bm25.py                           | BM25 full-text search via Neo4j FULLTEXT index. Returns ranked results with BM25 scores.                                                                             |             |
| omnirag/app/retrieval/vector.py                         | Vector similarity search via MongoDB Atlas Vector Search. Returns top k results with cosine similarity scores.                                                       |             |
| omnirag/app/retrieval/graph_expansion.py                | Neo4j graph traversal 1-2 hops from top retrieval results. Adds causally connected nodes to result set.                                                              |             |
| omnirag/app/retrieval/rrf.py                            | Reciprocal Rank Fusion — fuses BM25 and vector rankings into single ranked list. Pure function.                                                                      |             |
| omnirag/app/retrieval/temporal_filter.py                | Ranks down outdated nodes for current-state queries. Applies temporal filter based on query type.                                                                    |             |
| omnirag/app/retrieval/hallucination_guard.py            | Cross-references every claim in generated answer against source nodes. Strips uncorroborated claims. Core AI safety component.                                       | R-90, R-95  |
| omnirag/app/retrieval/confidence_scorer.py              | Derives confidence score from graph properties — source_count, recency, contradiction_count, verification_status. Never from LLM self-report.                        | R-92        |
| omnirag/app/retrieval/test_retrieval.py                 | Unit tests including test that hallucination guard strips claims with no source node.                                                                                | R-17, R-102 |
| omnirag/app/agents/                                     | LangGraph agent pipeline                                                                                                                                             |             |
| omnirag/app/agents/coordinator.py                       | LangGraph StateGraph — coordinator node, specialist routing, five-step chain, self-reflection step.                                                                  |             |
| omnirag/app/agents/graph_traversal_specialist.py        | Factual queries — direct lookups, relationship traversal, decision history.                                                                                          |             |
| omnirag/app/agents/causal_inference_specialist.py       | Causal and "why" queries — traces causal chains, identifies decision sequences.                                                                                      |             |
| omnirag/app/agents/synthesis_specialist.py              | Complex multi-source queries — assembles findings from multiple retrieval results.                                                                                   |             |
| omnirag/app/agents/tools.py                             | Six named tools: search_knowledge_graph, find_causal_chain, get_decision_history, find_expert, detect_contradictions, get_temporal_state. LLM decides which to call. |             |
| omnirag/app/agents/test_agents.py                       | Tests that low confidence escalates. Tests that self-reflection step catches incomplete answers.                                                                     | R-102       |
| omnirag/app/intelligence/                               | High-level intelligence features built on top of graph + retrieval                                                                                                   |             |
| omnirag/app/intelligence/drift_detector.py              | Compares concept node state at two timestamps. Calls Groq to narrate what changed.                                                                                   |             |
| omnirag/app/intelligence/gap_detector.py                | Queries unanswered Question nodes above ask_count threshold. Identifies undocumented high-traffic concepts.                                                          |             |
| omnirag/app/intelligence/expert_router.py               | Computes contribution-weighted expertise scores per entity per concept. Returns ranked expert list.                                                                  |             |
| omnirag/app/intelligence/ktd_generator.py               | Knowledge Transfer Document generator — triggered by contributor activity drop, compiles contribution history via LangChain.                                         |             |
| omnirag/app/intelligence/community_verification.py      | Verification workflow — notify top 3 contributors, track approvals, promote trust tier on 2 of 3 approvals.                                                          |             |
| omnirag/app/auth/                                       | Google OAuth, JWT, Authlib, rate limiting middleware                                                                                                                 |             |
| omnirag/app/websocket/                                  | Server-Sent Events manager — Redis pub/sub bridge for streaming query tokens to React frontend                                                                       |             |
| omnirag/app/query/                                      | Query entry point — classifies query type, routes to retrieval or agent pipeline, assembles final response                                                           |             |
...
| omnirag/docker-compose.yml                              | 5 services: omnirag-api (Python/FastAPI), mongodb, neo4j, redis, ollama                                                                                              |             |
...
- Files: snake_case — entity_resolution/stage1_lexical.py,
  graph/contradiction_detector.py

- Pydantic Models: PascalCase, no prefix — GraphNode, QueryResult,
  UnifiedEvent

- Classes: PascalCase — EntityResolver, HallucinationGuard,
  TemporalKnowledgeGraph

- Functions and variables: snake_case — resolve_entity(),
  merge_confidence, neo4j_session

- Constants: UPPER_SNAKE_CASE in constants.py — ENTITY_MERGE_THRESHOLD,
  GRAPH_EXPANSION_HOPS

- FastAPI routers: snake_case filename, variable named router — router =
  APIRouter()

- Cypher query variables: snake_case — MATCH (concept_node:Concept),
  RETURN concept_node
...
- Singleton pattern — Neo4j driver, MongoDB connection,
  sentence-transformers model loaded once. Imported as module-level
  singleton.
...
- Dependency injection via FastAPI's Depends() and function parameters —
  repositories passed to service functions as parameters, never imported
  directly inside service logic. Enables testing with mocks.
...
- Catching bare exceptions without handling — except Exception: print(e)
  or bare except: never committed
...
- Circular imports — if A imports B and B imports A, extract shared
  types to shared/schemas.py

- Missing type hints — every function parameter and return value has an
  explicit type hint, mypy strict mode enforced
...
4.  Agents — LangGraph coordination and LLM calls. Calls retrieval
...
- All CI checks pass — Ruff lint, mypy strict compile, pytest unit
  tests, integration tests

- No print() anywhere — use structlog logger
...
10. Simple queries before LangGraph — verify retrieval quality before
    adding agent complexity

11. LangGraph agents after retrieval — coordinator calls retrieval as a
    tool
...
- Repository with folder structure from 3.1 — empty Python files with
  docstring module descriptions

- .gitignore (\_\_pycache\_\_, .env, .venv, dist), .env.example (all
  variables with descriptions), README skeleton

- Pre-commit hooks — Ruff lint, mypy strict compile, no-secrets scanner

- GitHub Actions CI skeleton — runs on every push: lint, type check,
  pytest (even with no tests)

- docker-compose.yml with all 5 services: mongodb, neo4j (4.4 with
  APOC), redis, omnirag-api, ollama

- Multi-stage Dockerfile: dev stage (uvicorn --reload), prod stage
...
> **PHASE 2 Backend Skeleton** *FastAPI app + all route stubs + error
> handling*

**Deliverables**

- FastAPI app factory in app/main.py — registers all routers,
  middleware, error handler

- Pydantic Settings singleton in app/config.py — validates all env vars
  on import, raises with clear message if missing

- structlog structured logger in app/shared/logger.py — all subsequent
  modules import from here

- secure-headers middleware, CORS middleware (frontend origin only),
...
- Pydantic models for all request and response shapes in
...
- All CI checks pass including mypy strict compile
...
- MongoDB collections created with Motor (async PyMongo) — raw_events,
...
- Google OAuth flow via Authlib's Google OAuth client — POST
  /auth/google exchanges code for user profile
...
- slowapi (FastAPI rate limiting) on all public endpoints — values from
  Section 2.4
...
- GitHub webhook handler (PyGithub for API calls) — validates signature,
  extracts push/PR/issue/review events, publishes to Redis Streams XADD

- Slack Bolt for Python event listener — message and reaction events,
  thread hierarchy preserved in payload

- File upload endpoint — pypdf for PDF, python-docx for DOCX, text
...
- Three-stage entity resolution pipeline in
  app/entity_resolution/resolver.py — Stage 1 Jaro-Winkler, Stage 2
  sentence-transformers embedding cosine similarity, Stage 3 Neo4j graph
  neighborhood

- sentence-transformers all-MiniLM-L6-v2 model loaded as singleton on
  server startup — model warmup so first call is not slow
...
- sentence-transformers model warm on startup — first embedding call \<
  200ms
...
- RRF fusion in app/retrieval/rrf.py — combines BM25 and vector
  rankings. Pure function, easy to unit test.

- Graph expansion in app/retrieval/graph_expansion.py — 1-2 hop Neo4j
...
- Hallucination guard in app/retrieval/hallucination_guard.py —
...
>
> **PHASE 8 LangGraph Agent Pipeline** *Coordinator + 3 specialists +
> tools + five-step chain*

**Deliverables**

- LangGraph StateGraph in app/agents/coordinator.py — QueryState schema
  (Pydantic/TypedDict), coordinator node, specialist routing nodes,
...
- Six named tools in app/agents/tools.py: search_knowledge_graph,
...
> **→** *LangGraph StateGraph state mutation is the biggest source of
...
- Knowledge Transfer Document trigger — Celery periodic task (Python's
  native task queue, no longer just an "equivalent") checks
  Entity.last_active_at daily. Triggers KTD generation when contributor
  silent for threshold days.

- KTD generator — LangChain chain compiles: contribution history,
...
- sentence-transformers embedding batch processing — embed multiple
  texts in single call where possible

- N+1 query detection — audit all retrieval paths for multiple
  sequential single-node lookups

- Cache hit rate logging — log hits/misses per query type in structlog
  structured logs
...
- Structured logging audit — every key operation has a structlog log
  with request_id
...
- Security hardening — OWASP checklist from 2.5 verified, pip-audit
  clean
...
- Zero pip-audit critical or high vulnerabilities
...
| 2        | 1–2           | Repository with folder structure, pre-commit hooks, CI passing, FastAPI skeleton with all 18 route stubs, mypy strict compile passing            | Every endpoint returns 501 with standard envelope. CI green. mypy compiles clean.                                 | Neo4j Docker image first pull is slow and large — pre-pull before demo day                                  |
| 3        | 3             | All MongoDB collections with Motor (async PyMongo), Neo4j schema constraints and indexes, all repository modules, seed script, integration tests | Seed script creates correct data. Temporal query test passes. All repository tests passing.                       | Neo4j full-text search index creation syntax — test in Browser first                                        |
| 4        | 4–5           | Google OAuth, JWT, webhook signature validation, GitHub webhook ingestion, Slack ingestion, file upload, Redis Streams consumer                  | Integration test: GitHub push event in MongoDB within 5 seconds                                                   | Redis Streams consumer group acknowledgment logic — most common debugging trap in async Python              |
| 5        | 6 first half  | sentence-transformers model loaded, Stage 1 Jaro-Winkler, Stage 2 embedding similarity, Stage 3 graph neighborhood, adversarial test suite       | Entity resolution adversarial test \< 2% false merge rate                                                         | sentence-transformers model download size in Docker — add to Dockerfile pre-download step                   |
...
| 8        | 8             | LangGraph coordinator + 3 specialists, 6 tools, five-step chain with self-reflection, agent status events to SSE                                 | Complex causal query returns cited answer within 45 seconds                                                       | LangGraph StateGraph state mutation bugs — test each node in isolation first                                |
...
> **PART 05 — DevOps & Deployment Blueprint**
>
> **⚠** *OmniRAG has 5 services in Docker Compose — Python API, MongoDB,
> Neo4j, Redis, Ollama. The Neo4j and Ollama images are large (2-3GB
> combined). Pre-pull these images before any demo. docker-compose up on
> first run takes 5-10 minutes on a slow connection.*
>
> **5.1 — Docker & Environment Strategy**

**Docker Strategy**

Multi-stage Dockerfile. Stage 1 (builder): python:3.12-slim, installs
all dependencies via pip including dev/test tools. Stage 2 (production):
python:3.12-slim, copies only installed site-packages and application
code (production only). Smaller production image, no test frameworks or
...
| Python execution | uvicorn --reload (no compile step) | uvicorn + gunicorn workers | uvicorn + gunicorn workers                               |
...
|----------------------|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------|-----------------|
| 1: Quality Gate      | Every push to every branch                | Checkout → Python 3.12 setup → pip install -r requirements.txt (cached) → Ruff lint → mypy strict compile → pytest unit tests                                         | Block PR merge. Comment on PR with failing check.  | \< 3 min        |
...
graph inspection. Production: FastAPI (Python) API on Render free tier
(spins down after inactivity — warn in demo). React frontend on Vercel
(no spin-down). MongoDB Atlas free tier, Neo4j AuraDB free tier, Upstash
Redis free tier.
...
| Groq rate limit causing slow queries             | Check structlog logs for 429 responses. Count LLM calls in last minute.                              | Ollama fallback should activate. If not activating, check OLLAMA_BASE_URL env var.        | In production: wait 60 seconds for Groq reset. Locally: verify Ollama running in Docker                   |
...
sentence-transformers models are downloaded from Hugging Face Hub — not
backed up, re-downloaded on deployment. Ollama llama3: redownloaded on
container restart. No custom-trained models in OmniRAG — all inference
is pre-trained.
...
| PORT                   | API server port                                             | 3001                                                | Required     |
...
| Phase 0  | Embedding model                          | sentence-transformers all-MiniLM-L6-v2       | OpenAI text-embedding-3-small    | Zero API cost, zero network latency, works offline, runs in same Python process. Critical for entity resolution performance.                   |
...
| Phase 0  | Agent framework                          | LangGraph                                    | LangChain sequential chain       | LangGraph handles stateful graph-based coordination — coordinator decides dynamically. LangChain sequential chain is fixed order.              |
...
| Phase 3 (post) | MongoDB local dev engine | mongodb/mongodb-atlas-local (single container, bundled mongod+mongot+runner) | Two-container mongodb-community-server + mongodb-community-search sidecar | Two-container wiring never got an auth user configured between mongod and mongot and failed at the mongot binary/startup stage before that gap even mattered. atlas-local is MongoDB's own documented single-container replacement for this exact use case — same $vectorSearch capability, no manual sync-source wiring. See CHANGELOG for full detail. |
| Phase 3 (post), reversed same session | MongoDB local dev engine | mongo:7 (plain, no bundled Atlas Search/mongot) | mongodb/mongodb-atlas-local | atlas-local reliably failed to start on this dev machine (Windows 11, Docker Desktop, WSL2 backend) with a keyfile-generation error (`Read security file failed ... /data/configdb/keyfile: No such file or directory` → `Unable to acquire security key[s]` → panic/exit 2), reproduced on fresh volumes, no volumes, with/without an init-script mount, and after a full `docker compose down -v`. Confirmed via `docker logs -f` that mongod starts normally for several seconds before dying — not a startup race. A similar but distinct community-reported bug exists for this image (fails only after restart, not first boot); ours failed on first boot even with all documented-required volumes present, so root cause is not conclusively identified — see full writeup in CHANGELOG. Plain `mongo:7` started healthy on first try with zero special config. **Consequence: local Atlas Vector Search capability is currently unavailable — Phase 7 retrieval work planned against it has no local backing store until this is revisited (retry on non-Windows host, swap to Qdrant, or retest a newer atlas-local release).** |
| Phase 3 (post) | Redis host port mapping | 6380:6379 (host:container) | 6379:6379 | Host port 6379 was already bound by an unrelated project's Redis container on this dev machine, causing `docker compose up` to fail with "port is already allocated." Container-internal port and `REDIS_URL` env var (`redis://redis:6379/0`, addressing the Docker-internal network) are unchanged — only the host-side publish port moved. Host-specific; may not apply on a machine without the collision, but harmless either way. |
...
|----------|---------------------------------------------------------------------------------------------------------------------|-----------------------------|--------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| DEBT-001 | Knowledge Spaces multi-tenancy — spaceId partitioning on Neo4j graph labels and MongoDB collections not implemented | app/database/, neo4j schema | Core graph features take priority in v1                                  | Add spaceId: string property to all nodes. Partition Neo4j with workspace label prefixes. Add JWT space-scoped claims.                      | Medium       |
| DEBT-002 | Discord, Notion, Google Drive connectors — only 3 sources in v1                                                     | app/ingestion/              | Three sources done deeply is stronger than ten sources shallowly         | Add connector per source after v1 proven. Each connector follows the same UnifiedEvent normaliser pattern.                                  | Medium       |
| DEBT-003 | Socket.io Live Knowledge Rooms — real-time collaborative querying not built                                         | app/websocket/              | Too complex — real-time + graph updates + room management simultaneously | After core query pipeline stable. Build as progressive enhancement on top of existing SSE stream.                                           | Low          |
...
| DEBT-005 | Onboarding Intelligence — dependency-ordered concept paths for new members                                          | app/intelligence/           | Overlaps with KTD feature in v1                                          | Build on top of expert routing and EXPERTISE_IN graph in v2 after KTD system proven.                                                        | Low          |
| DEBT-006 | Prometheus + Grafana observability — not included in v1                                                             | monitoring/                 | Not core story for OmniRAG unlike SentinelAI                             | Add a structlog-based Prometheus exporter and basic Grafana dashboard in v2 if SentinelAI observability skills need demonstrating here too. | Low          |
| DEBT-007 | MongoDB Atlas Vector Search unavailable in local dev — mongodb-atlas-local fails to start reliably on this dev host (Windows 11 / Docker Desktop / WSL2), reverted to plain mongo:7 which has no bundled Atlas Search/mongot process at all | docker-compose.yml, app/retrieval/vector.py (Phase 7, not yet built) | Root cause not conclusively identified (see CHANGELOG for full investigation); plain mongo:7 unblocks everything else immediately, so this was not worth more debugging time this session | Before Phase 7 hybrid retrieval work begins: either (a) retry mongodb-atlas-local on a non-Windows or non-WSL2 host to isolate whether this is host-specific, (b) swap the vector store to Qdrant per Blueprint 2.2's own rejected-alternatives list, accepting the extra container/connection cost, or (c) retest a newer mongodb-atlas-local release, since the image was new at time of writing | High — blocks Phase 7 as currently scoped |
...
Model: sentence-transformers/all-MiniLM-L6-v2 via sentence-transformers
v3. Dimensions: 384. Similarity metric: cosine. Loaded as singleton on
server startup. Model version pinned in requirements.txt — never
auto-upgrade.
...
sentence-transformers vector search via MongoDB Atlas Vector Search run
...
| Retrieval latency p95                | \< 200ms for full hybrid retrieval pipeline                                                       | Logged per query in structlog structured logs — timing wrapper around retrieval module | Exceeds 500ms consistently                                                          |
...
| Agent pipeline completion rate       | \> 95% of queries complete without timeout or error                                               | Error rate logged per query type in structlog logs                                     | Falls below 90% — Groq availability or timeout issue                                |
...
Seed script documentation — app/database/seeds/README.md explains every
...
The HallucinationGuard in app/retrieval/hallucination_guard.py is a
deterministic Python function — no LLM involved. It runs on every
...
The ConfidenceScorer in app/retrieval/confidence_scorer.py is a
deterministic Python formula. Never trusts LLM self-reported confidence
(Rule R-92).
